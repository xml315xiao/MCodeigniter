...
class MY_CURD extends CI_Model
{
...
//        $this->load->helper('inflector');
    ...
    /**
     * 根据主键值查找单挑数据
     * @param  array $condition 只支持and条件查询
     * @param  string $field
     * @param  $order_by
     * @return
     */
    public function get_row($field = '*', $condition = [], $order_by = [], $table = '')
    {
        ...
        if (is_array($order_by) && sizeof($order_by) > 0) {
            foreach ($order_by as $index=>$direction) {
                $this->_database->order_by($index, $direction);
            }
        }

        $from_table = strlen($table) > 0 ? $table : $this->_table;
        return $this->_database->select($field)->get($from_table)->{$this->_return_type()}();
    }
     ...
     * @return array | object
     */
    public function fetch_lists($field = '*', $condition = [], $order_by = [])
    {
        if (is_array($field) && sizeof($field) > 0) {
            $field = implode(',', $field);
        }
        ...
        $this->_database->select($field)->get($this->_table)->{$this->_return_type()}();
    }
     ...
     * @return int
     */
    public function count_results($condition = [])
    {
        if (empty($condition)) {
            return $this->_database->count_all($this->_table);
        } else {
            return $this->_database->where($condition)->count_all_results($this->_table);
        }
    }
